...
app.get('/filter', (req, res) => {
  ...
  console.log(query);
  const filteredJokes = jokes.filter((joke) => joke.jokeType === query);
  const randIndex = Math.floor(Math.random() * filteredJokes.length);
  if (!filteredJokes.length) {
    res.status(404).json({ error: 'Joke not found' });
  }
  const randJoke = filteredJokes[randIndex];
  res.status(200).json(randJoke);
});
...
app.put('/jokes/:id', (req, res) => {
  ...
  const index = jokes.findIndex((joke) => joke.id === parseInt(id));
  if (index === -1) {
    res.status(404).json({ error: 'Joke not found' });
    return;
  }
...
app.patch('/jokes/:id', (req, res) => {
  ...
  res.status(201).json(patchedJoke);
});
